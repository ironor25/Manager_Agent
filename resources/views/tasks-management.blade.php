...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted mb-0">Create, assign, edit, and delete employee tasks</p>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            ...
            <div class="modal-body p-4">
                <h6 class=" mb-1" style="font-size: 0.8rem; text-transform: uppercase;">Task Title</h6>
                <h4 id="view_task_title" class="fw-bold  mb-4"></h4>
                
                <h6 class=" mb-2" style="font-size: 0.8rem; text-transform: uppercase;">Description</h6>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="d-flex align-items-center gap-3 p-3 border rounded-3 h-100">
                            ...
                            <div>
                                <h6 class=" mb-1" style="font-size: 0.8rem; text-transform: uppercase;">Assigned To</h6>
                    ...
                    <div class="col-md-6">
                        <div class="d-flex align-items-center gap-3 p-3 border rounded-3 h-100">
                            ...
                            <div>
                                <h6 class=" mb-1" style="font-size: 0.8rem; text-transform: uppercase;">Deadline</h6>
                    ...
                    <div class="col-md-6">
                        <div class="d-flex align-items-center gap-3 p-3 border rounded-3 h-100">
                            ...
                            <div>
                                <h6 class=" mb-1" style="font-size: 0.8rem; text-transform: uppercase;">Priority</h6>
                    ...
                    <div class="col-md-6">
                        <div class="d-flex align-items-center gap-3 p-3 border rounded-3 h-100">
                            ...
                            <div>
                                <h6 class=" mb-1" style="font-size: 0.8rem; text-transform: uppercase;">Status</h6>
                                <div id="view_task_status" class="fw-bold text-body mb-0"></div>
                            </div>
                        </div>
                    </div>
                    <div class="col-12" id="view_dependency_row" style="display: none;">
                        <div class="d-flex align-items-center gap-3 p-3 border rounded-3">
                            <div class="avatar bg-light text-warning fw-bold" style="width: 40px; height: 40px; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                                <i class="fa-solid fa-circle-exclamation"></i>
                            </div>
                            <div>
                                <h6 class=" mb-1" style="font-size: 0.8rem; text-transform: uppercase;">Blocked By (Dependency)</h6>
                                <div id="view_task_dependency" class="fw-bold text-body mb-0"></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="employee_id" class="form-select employee-search" required data-placeholder="Search for employee...">
                                <option value=""></option>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold text-body">Project (Optional)</label>
                            <select name="project_id" class="form-select project-search" data-placeholder="Select a Project...">
            ...
            <form id="editTaskForm" method="POST">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold text-body">Assign To</label>
                            <select name="employee_id" id="edit_employee_id" class="form-select employee-search" required data-placeholder="Search for employee...">
                                <option value=""></option>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold text-body">Project (Optional)</label>
                            <select name="project_id" id="edit_project_id" class="form-select project-search" data-placeholder="Select a Project...">
                                <option value="">No Project</option>
                                @foreach($projects as $proj)
                                    <option value="{{ $proj->id }}">{{ $proj->name }}</option>
                                @endforeach
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold text-body">Depends On (Dependency Task)</label>
                        <select name="depends_on_task_id" id="edit_depends_on_task_id" class="form-select task-search" data-placeholder="Search for a task it depends on (Optional)...">
                            <option value=""></option>
                        </select>
                    </div>
                    <div class="row">
...
<script>
    $(document).ready(function() {
            ...
            columns: [
                { 
                    ...
                    render: function(data, type, row) {
                        return '<div class="fw-bold text-body">' + data + '</div>';
                    }
                },
                { 
                    data: 'employee_name', 
                ...
                },
                {
                    ...
                    render: function(data) {
                        return '<span class="text-body fw-semibold">' + data + '</span>';
                    }
                },
                {
                    data: 'project_name',
                ...
                { 
                    ...
                    name: 'priority',
                    render: function(data, type, row) {
                        var priorityStr = data.charAt(0).toUpperCase() + data.slice(1);
                        return '<span class="priority-badge priority-' + data + '">' + priorityStr + '</span>';
                    }
                },
                { 
                    data: 'status', 
                    name: 'status',
                    render: function(data, type, row) {
                        var statusStr = data.replace('_', ' ');
                        statusStr = statusStr.charAt(0).toUpperCase() + statusStr.slice(1);
                        return '<span class="status-badge status-' + data + '">' + statusStr + '</span>';
                    }
                },
                { 
                    ...
                    render: function(data, type, row) {
                        ...
                        return '<span class="text-muted fst-italic">No deadline</span>';
                    }
                },
                {
        ...
        // Initialize Select2 with AJAX for employee selection
        ...
        // Initialize Select2 for task dependency search
            $('.task-search').select2({
                theme: 'bootstrap-5',
                allowClear: true,
                dropdownParent: function(elem) {
        ...
        // Use event delegation since buttons are generated by DataTables
        $('#tasks-table').on('click', '.edit-task-btn', function() {
            var task = $(this).data('task');
            
            $('#editTaskForm').attr('action', '/admin/tasks/' + task.id);
            $('#edit_title').val(task.title);
            $('#edit_description').val(task.description);
            
            // Employee Select2
            var empSelect = $('#edit_employee_id');
            if (empSelect.find("option[value='" + task.employee_id + "']").length) {
                empSelect.val(task.employee_id).trigger('change');
            } else { 
                var newOption = new Option(task.employee ? task.employee.name : 'Selected Employee', task.employee_id, true, true);
                empSelect.append(newOption).trigger('change');
            } 

            // Dependency Task Select2
            var depSelect = $('#edit_depends_on_task_id');
            depSelect.val(null).trigger('change');
            if (task.depends_on_task_id) {
                if (depSelect.find("option[value='" + task.depends_on_task_id + "']").length) {
                    depSelect.val(task.depends_on_task_id).trigger('change');
                } else {
                    // Fetch task details if possible, or label simply
                    var depOption = new Option(task.depends_on_task ? task.depends_on_task.title : 'Blocked Task #' + task.depends_on_task_id, task.depends_on_task_id, true, true);
                    depSelect.append(depOption).trigger('change');
                }
            }
            ...
            if (task.project_id) {
                ...
                $('#edit_project_id').val('').trigger('change');
            }
        ...
        $('#tasks-table tbody').on('click', '.task-row', function(e) {
            ...
            if (task.depends_on_task_id) {
                $('#view_dependency_row').show();
                $('#view_task_dependency').text(task.depends_on_task ? task.depends_on_task.title : 'Task #' + task.depends_on_task_id);
            } else {
                $('#view_dependency_row').hide();
            }
